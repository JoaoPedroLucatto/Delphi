...
var
  FCadTransfRR: TFCadTransfRR;
  Condicao : string;
  Erros : integer;

implementation

uses UDM, UPrin, Funcoes, TrataErros, URelTransfPropRR, UClasse_PesqRur,
     UClasse_PesqPrin;

{$R *.dfm}

function TFCadTransfRR.Pesquisa(ini, fim : string) : boolean;
var onde : string;
    qrx : TSQLQuery;
begin
 onde := '';
...
procedure TFCadTransfRR.ClicaGrid;
begin
  cdsCadastro.Close;
...
procedure TFCadTransfRR.BtnAnteriorClick(Sender: TObject);
begin
 if cdsVis.Active then cdsVis.Prior;
 ClicaGrid;
end;

procedure TFCadTransfRR.BtnPrimeiroClick(Sender: TObject);
begin
 if cdsVis.Active then cdsVis.First;
 ClicaGrid;
end;

procedure TFCadTransfRR.BtnProximoClick(Sender: TObject);
begin
 if cdsVis.Active then cdsVis.Next;
 ClicaGrid;
end;

procedure TFCadTransfRR.BtnUltimoClick(Sender: TObject);
begin
 if cdsVis.Active then cdsVis.Last;
 ClicaGrid;
end;

procedure TFCadTransfRR.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 fecha_todos_cds(TForm(Sender));
 Action := caFree;
end;

procedure TFCadTransfRR.FormDestroy(Sender: TObject);
begin
 TForm(Sender) := Nil;
end;

procedure TFCadTransfRR.FormShow(Sender: TObject);
begin
 PG.ActivePageIndex  := 0;
 desabilita(FCadTransfRR);
...
procedure TFCadTransfRR.DBGrid1CellClick(Column: TColumn);
begin
 ClicaGrid;
end;

procedure TFCadTransfRR.DBGrid1DblClick(Sender: TObject);
begin
 ClicaGrid;
 PG.ActivePageIndex  := 0;
end;

procedure TFCadTransfRR.DBGrid1TitleClick(Column: TColumn);
begin
  OrdenarGRID(cdsVis, dsVis, Column);
end;

procedure TFCadTransfRR.btnPropAntigoClick(Sender: TObject);
...
procedure TFCadTransfRR.btnPropNovoClick(Sender: TObject);
var Pesq : TPesq_CodCNT;
begin
 Pesq := TPesq_CodCNT.Create;
 if Pesq.BotaoCodCNT then begin
...
procedure TFCadTransfRR.DBEdit1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TFCadTransfRR.DBEdit1Enter(Sender: TObject);
begin
...
procedure TFCadTransfRR.DBEdit2Enter(Sender: TObject);
begin
 if not (cdsCadastro.State in [dsInsert, dsEdit]) then exit;
...
procedure TFCadTransfRR.DBEdit2Exit(Sender: TObject);
var Pesq : TPesq_CodCNT;
begin
...
procedure TFCadTransfRR.DBEdit1Exit(Sender: TObject);
begin
 if BtnCancela.Focused then exit;
 if not (cdsCadastro.State in [dsInsert, dsEdit]) then exit;
 ...
 If trim(DBEdit1.Text) <> '' Then begin
  cdsCadastro.FieldByName('COD_RRR_ANT_RTP').AsString := MontaMascara(DBEdit1.Text, 'RUR', SIA_Empresa);
  if not Pesquisa(cdsCadastro.FieldByName('COD_RRR_ANT_RTP').AsString,'') then btnPropAntigo.Click;
 end;

 if trim(DBEdit1.Text) <> '' then begin
  if trim(DBEdit2.Text) = trim(DBEdit8.Text) then begin
    showmessage('O Código Antigo do Proprietário não pode ser igual ao Novo.');
...
procedure TFCadTransfRR.btnPesquisaClick(Sender: TObject);
Var SQL : String;
Begin
...
procedure TFCadTransfRR.BtnNovoClick(Sender: TObject);
begin
  Condicao := 'Incluindo';
  abilita(FCadTransfRR);
...
procedure TFCadTransfRR.BtnExcluirClick(Sender: TObject);
var qrx : TSQLQuery;
    aux : string;
begin
  ...
  if dm.msg.MessageDlg('Deseja Realmente Cancelar esta Transferência ?',mtConfirmation,[mbyes,mbno],0) = mryes then begin
    qrx := TSQLQuery.Create(Application.Owner);
    qrx.SQLConnection := dm.Conexao;
    ...
    if dm.msg.MessageDlg('Atenção !!!'+#13+#13+
...
procedure TFCadTransfRR.BtnCancelaClick(Sender: TObject);
begin
  desabilita(FCadTransfRR);
...
procedure TFCadTransfRR.BtnConfirmaClick(Sender: TObject);
var qrx : TSQLQuery;
    aux : string;
begin
  PG.ActivePageIndex := 0;
  ...
  if trim(DBEdit1.Text) = '' then begin
      dm.msg.MessageDlg('O campo Código Antigo é obrigatório. Você deve informar um número!!!',mtError, [mbok], 0);
      DBEdit1.SetFocus;
      Exit;
  end;

  if trim(DBEdit2.Text) = '' then begin
      dm.msg.MessageDlg('O campo Código Novo é obrigatório. Você deve informar um número!!!',mtError, [mbok], 0);
      DBEdit2.SetFocus;
      Exit;
  end;

  if trim(DBEdit3.Text) = '' then begin
      dm.msg.MessageDlg('O campo Motivo é obrigatório. Você deve informar um número!!!',mtError, [mbok], 0);
      DBEdit3.SetFocus;
      Exit;
  end;
  ...
  if Condicao = 'Incluindo' then begin
    ...
    qrx := TSQLQuery.Create(Application.Owner);
    qrx.SQLConnection := dm.Conexao;
    qrx.SQL.Text := 'UPDATE RR_CAD_RURAL SET COD_CNT_RRR = '+''''+ cdsCadastro.FieldByName('COD_CNTV_RTP').AsString +''''+
                    ' WHERE COD_EMP_RRR = '+ SIA_Empresa +
                      ' AND COD_RRR = '+''''+ cdsCadastro.FieldByName('COD_RRR_ANT_RTP').AsString +'''';
    qrx.ExecSQL;
    FreeAndNil(qrx);
...
procedure TFCadTransfRR.BtnSairClick(Sender: TObject);
begin
 close;
 if FormStyle = fsMDIChild then FPrin.AbreMenu;  
end;

procedure TFCadTransfRR.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure TFCadTransfRR.cdsCadastroReconcileError( DataSet: TCustomClientDataSet; E: EReconcileError; UpdateKind: TUpdateKind; var Action: TReconcileAction);
begin
  if UpdateKind = ukDelete then Action := raCancel
  else Action := raCorrect;
end;

procedure TFCadTransfRR.cdsCadastroNewRecord(DataSet: TDataSet);
begin
...
procedure TFCadTransfRR.dspCadastroBeforeUpdateRecord(Sender: TObject; SourceDS: TDataSet; DeltaDS: TCustomClientDataSet; UpdateKind: TUpdateKind; var Applied: Boolean);
begin
...
procedure TFCadTransfRR.dspCadastroGetTableName(Sender: TObject; DataSet: TDataSet; var TableName: WideString);
begin
 TableName := 'RR_TRANSPROP';
end;

procedure TFCadTransfRR.dspCadastroUpdateError(Sender: TObject; DataSet: TCustomClientDataSet; E: EUpdateError; UpdateKind: TUpdateKind; var Response: TResolverResponse);
begin
  Tratar_Erro(E.Message);
end;

procedure TFCadTransfRR.dspCadastroAfterUpdateRecord(Sender: TObject;
  SourceDS: TDataSet; DeltaDS: TCustomClientDataSet;
  UpdateKind: TUpdateKind);
begin
 dm.Sia_Log(SIA_Usuario, 'RR_TRANSPROP', SIA_Modulo, UpperCase(FCadTransfRR.Caption), DeltaDS, UpdateKind);
end;

procedure TFCadTransfRR.FormKeyPress(Sender: TObject; var Key: Char);
begin
 if key = #27 then
  if not BtnCancela.Enabled then BtnSair.Click;
end;

procedure TFCadTransfRR.BtnImprimirClick(Sender: TObject);
begin
  if cdsCadastro.IsEmpty then PG.ActivePageIndex := 1;
  if cdsCadastro.IsEmpty then Exit;

  if cdsCadastro.FieldByName('STATUS_RTP').AsString <> 'AT' then begin
    dm.msg.MessageDlg('Só é permitido a impressão que apresenta Status de ATIVO.', mtInformation, [mbok], 0);
    exit;
  end;

  Application.CreateForm(TFRelTransfPropRR, FRelTransfPropRR);
  SIA_Aplicacao := 'FRelTransfPropRR';
  dm.Permissao(FRelTransfPropRR, 'RUR_FRelTransfPropRR', SIA_Empresa, SIA_Usuario, SIA_Modulo, SIA_Representante);
  with FRelTransfPropRR do begin
   CODRTP := '';
   if not cdsCadastro.IsEmpty then
     CODRTP := cdsCadastro.FieldByName('COD_RTP').AsString;
  end;
  FRelTransfPropRR.ShowModal;
  FRelTransfPropRR.Free;
end;
