...
begin
   codsetorvencimento:='1';  //está na tabela TRIVEN
...
begin
  ...
  begin
     ...
     if not QVerifica.IsEmpty then begin
          ...
          cdsdestino.fieldByName('EXIGIBILIDADEISS_NFS').value      :='Não Incidência'
          else
          cdsdestino.fieldByName('EXIGIBILIDADEISS_NFS').value      :='Exigível';

          cdsdestino.fieldByName('REGIMEESPECIALTRIB_NFS').value      :='Não Possui';
          ...
          if (trim(QNOrigem.fieldByName('nfeconsupersimples').AsString)='S') then begin
            ...
            cdsdestino.fieldByName('REGIMEESPECIALTRIB_NFS').value      :='Microempresário e Empresa de Pequeno Porte (ME EPP)';
          end;
          ...
          begin
              cdsdestino.fieldByName('REGIMEESPECIALTRIB_NFS').value      :='Microempresário Individual (MEI)';
              cdsdestino.fieldByName('TIPOISS_NFS').value      :='04 - Fixo';
              cdsdestino.fieldByName('OPTANTESIMPLES_NFS').AsString:='S';
          end;
...
begin
...

  {
    ...
    begin
       ...
       codaux := '..: Cobrança Amigável :..'+#13+
                 'Data Lançamento: '+ vartostr(QOrigem.FieldValues['AMI_D_LANC']) +#13+
                 'Data Vencimento: '+ vartostr(QOrigem.FieldValues['AMI_D_VEN']) +#13+
                 'Valor: '+ vartostr(QOrigem.FieldValues['AMI_N_VAL']) +#13+
                 'Observações: '+ iif(trim(vartostr(QOrigem.FieldValues['AMI_A_DES'])) <> '', vartostr(QOrigem.FieldValues['AMI_A_DES']) +#13, '') +
...
begin
    lbFuncao.Caption:='HistoricoAlteraçõesContribuintes';
    lbFuncao.update;
...
begin
     ...
     lbFuncao.Caption:='CodigoCobranças';
     lbFuncao.update;
    ...
    QOrigem.sql.add('SELECT DISTINCT TRIISEN.ISE_A_TIPO, '+
                                            'CASE TRIISEN.ISE_A_TIPO '+
                                             ...
                                             ' WHEN ''A'' THEN ''REDUÇÃO DE ALIQUOTA'' '+
                                             ' WHEN ''D'' THEN ''DEDUÇÃO'' '+
                                            'END DESCRICAO '+
                           'FROM TRIISEN '+
...
begin
    ...
    begin
        ...
        cdsdestino.fieldByName('DESCR_TEG').AsString   :='Próprio Imóvel'
        else
        if QOrigem.fieldByName('END_A_TIP_ENT').AsString='1' then
        cdsdestino.fieldByName('DESCR_TEG').AsString   :='Proprietário'
        else
        if QOrigem.fieldByName('END_A_TIP_ENT').AsString='2' then
        cdsdestino.fieldByName('DESCR_TEG').AsString   :='Compromissário'
        else
        if QOrigem.fieldByName('END_A_TIP_ENT').AsString='3' then
        cdsdestino.fieldByName('DESCR_TEG').AsString   :='Imobiliária'
        else
...
begin
    lbFuncao.Caption:='ServiçosUrbano';
    lbFuncao.update;
...
begin
    ...
    begin
      ...
      cdsdestino.fieldByName('COD_IMOBILIARIA_IPT').AsString:=ZeraEsquerda(9,QOrigem.fieldByname('CNTI_I_IMOBILIARIA').AsString);
       ...
       //se nao existir definição de endereço de correspondencia jogo do imovel
...
begin
  ...
  QOrigem.SQL.Add('SELECT TRIIMO.IMO_I_COD as COD_IPT_SOC, '+
                                   ...
                                   'CASE '+
                                    ' WHEN PROPIMOV.PRI_A_RES = 1 THEN ''SÓCIO RESPONSÁVEL PELO IMÓVEL'' '+
                                    ' WHEN PROPIMOV.PRI_A_RES = 2 THEN ''APENAS SÓCIO'' '+
                                    ' WHEN PROPIMOV.PRI_A_RES = 3 THEN ''PROMISSÁRIO / COMPROMISSÁRIO'' '+
                                    ' WHEN PROPIMOV.PRI_A_RES = 4 THEN ''TUTOR'' '+
                                    ' WHEN PROPIMOV.PRI_A_RES = 5 THEN ''COMODATÁRIO'' '+
...
begin
  ...
  lbFuncao.Caption:='Codigo de Cobrança do Imobiliario';
  lbFuncao.update;
...
begin
  ...
  QOrigem.SQL.Text :=' SELECT TRIIMO.IMO_I_COD as COD_IPT_SOC, '+
                     ...
                     '          CASE '+
                     '            WHEN A.PRI_A_RES = 1 THEN ''SÓCIO RESPONSÁVEL PELO IMÓVEL'''+
                     '            WHEN A.PRI_A_RES = 2 THEN ''APENAS SÓCIO'''+
                     '            WHEN A.PRI_A_RES = 3 THEN ''PROMISSÁRIO / COMPROMISSÁRIO'''+
                     '            WHEN A.PRI_A_RES = 4 THEN ''TUTOR'''+
                     '            WHEN A.PRI_A_RES = 5 THEN ''COMODATÁRIO'''+
...
begin
    ...
    begin
      ...
      cdsdestino.ApplyUpdates(-1);
          ...
          //cdsdestino.fieldByName('TIPVALOR_VRC').AsString:='07 - % da Taxa de Licença';
...

   {
    ...
    begin
      ...
      cdsdestino.fieldByName('COD_EMP_CAL').AsString:='1';
      cdsdestino.fieldByName('ABREVATIV_CAL').AsString:='Licença e Fiscalização '+ QOrigem.fieldByname('valor').AsString+' UFM ';
      cdsdestino.fieldByName('DESCRICAO_CAL').AsString:='Licença e Fiscalização '+ QOrigem.fieldByname('valor').AsString+' UFM ';
...
begin
    ...
    cdsdestino.FieldByName('DESCRI_TEM').AsString:='Não classificada';
    cdsDestino.post;
    ...
    cdsdestino.FieldByName('DESCRI_TEM').AsString:='Empresa de médio porte (EMP)';
    cdsDestino.post;
...
begin
    ...
    cdsdestino.FieldByName('COD_IND_STR').AsString:='1';
    cdsdestino.FieldByName('LI_RESPONSA_STR').AsString:='SETOR ÚNICO';
    cdsDestino.post;
    cdsdestino.ApplyUpdates(-1);
    end;
         ...
         codatd:=ZeraEsquerda(7,copy(codaux,4,2));
          ...
          if cdsdestino.IsEmpty then begin
            ...
            if QOrigem.FieldByName('cdGrupoAtividade').AsString='4' then
             cdsdestino.fieldByName('TIPOATIVIDADE_ATV').AsString:='01 - Prestação de Serviço'
            else
...
{
01 - Prestação de Serviço
02 - Licença
03 - Ambulante
04 - Horário Especial
05 - Publicidade
06 - Vigilância Sanitária
07 - Ocupação
08 - Outros
09 - Informativo
}
...
begin
  ...
  if cdsdestino.IsEmpty then
   ...
   cdsDestino.FieldByName('REGIMEESPECIALTRIB_MBL').AsString:='Não Possui';
   cdsDestino.FieldByName('TIPOISS_MBL').AsString:='01 - Não Possui';
   ...
   begin
    ...
    cdsDestino.FieldByName('REGIMEESPECIALTRIB_MBL').AsString:='Microempresário (ME)';
    ...
    cdsDestino.FieldByName('REGIMEESPECIALTRIB_MBL').AsString:='Microempresário Individual (MEI)';
   end;
...
//    where CAR_S_TIP=8 and b.CAR_S_COD=42
   ...
   cdsDestino.FieldByName('EXIGIBILIDADEISS_MBL').AsString:='Exigível';
   end;
   ...
   //cdsDestino.FieldByName('TIPOISS_MBL').AsString:='03 - Sobre Faturamento';
   //cdsDestino.FieldByName('TIPOISS_MBL').AsString:='04 - Fixo';
   //cdsDestino.FieldByName('TIPOISS_MBL').AsString:='01 - Não Possui ISSQN';
...
Begin
  ...
  SalvaFinanceiroBaixa_FI_PARCELA(cdsParcelas.fieldByName('COD_EMP_PAR').Value,
    ...
    While Not cdsReceitas.Eof Do Begin
      ...
      if numerodereceitas=contreceitas then begin // acerta as diferenças no ultimo registro
        pvalor_frc   := pvalor_frc - (desconto-diferdesconto) ;
        pdescor_frc  := pdescor_frc + (desconto-diferdesconto) ;
        pvrcorre_frc := pvrcorre_frc + (correcao-difercorrecao) ;
        pvrmulta_frc := pvrmulta_frc + (multa-difermulta) ;
...
begin
        ...
        cdsdestino.FieldByName('DESCR_FDS').AsString:='COLOCAR A DESCRIÇÃO CORRETA';
   ...
   lbItem.Update;
         ...
         begin
          ...
          if QAuxiliar.IsEmpty then begin
           ...
           if QOrigem.FieldByName('PROT_S_SITUACAO').AsString='6' then //6 - LiquidaÃ§Ã£o - Cheque
           begin
             status:='Q';
             protestado:='S';
           end
           ...
           if QOrigem.FieldByName('PROT_S_SITUACAO').AsString='8' then//9 - ExcluÃ­da do Protesto
           begin
             status:='D';
           end
           else
           if QOrigem.FieldByName('PROT_S_SITUACAO').AsString='10' then//10 - ExcluÃ­da do Protesto
           begin
             status:='D';
           end
           else
           ...
           if QOrigem.FieldByName('PROT_S_SITUACAO').AsString='12' then//12 - DesistÃªncia Confirmada
           begin
             status:='D';
           end
           else
